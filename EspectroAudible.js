/*
    ...
    En este ejemplo se han utilizado las siguientes canciones :
...
EspectroAudible.prototype = Object.assign( Object.create(ObjetoCanvas.prototype) , {
    ...
    MouseSoltado    : function(Evento) { 
        ...
        if (this.C_BarraTiempo.Hover === true) {
            var ct = (this.Audio.TiempoTotal() / this.AnchoBarraTiempo) * this.C_BarraTiempo.PosHover;
            this.Audio.NuevoTiempoActual(ct);             
        }
...
//        this.Escena.fog = new THREE.Fog(0x888888, 10.1, 150);
        ...
        var fftSize = (ObjetoNavegador.EsMovil() === false) ? 2048 : 512;
        this.PrecisionAnalizador = (fftSize === 2048) ? 0 : 1;
        
        this.Audio = new EspectroAudible_Audio();
        this.Audio.Iniciar(fftSize, this, 
        ...
        this.ReIniciarCubos(Math.sqrt(fftSize / 2));    
...
//        this.BufferIcos[0].Context.fillRect(0, 0, this.BufferIcos[0].Ancho, this.BufferIcos[0].Alto);
        ...
        this.C_Play.add(this.PlanoTextura[2]); // pausa
        this.PlanoTextura[2].visible = false; // pausa invisible
    ...
    ActualizarTiempoSlider : function() {
        var TA = this.Audio.TiempoActual();
        var TT = this.Audio.TiempoTotal();
        var TMins = Math.round(Math.floor(TT) / 60);
        var TSecs = (TT - (TMins * 60)).toFixed(0);
        var T = TMins + ((TSecs < 10) ? ':0' : ':') + TSecs;
        var AMins = Math.round(Math.floor(TA / 60));
        var ASecs = Math.round(TA - (AMins * 60));
        ...
        var Parte = this.AnchoBarraTiempo / TT;
        // Posición del slider
        this.C_SliderTiempo.position.x = -(this.AnchoBarraTiempo / 2) + (TA * Parte);
    ...
    MediaFrequenciasAudio : function() {
        ...
        var HzPorBarra = this.Audio.Context.sampleRate / this.Audio.Analizador.fftSize;
    ...
    PintarPantalla_OndaSin : function(Buffer, Tick) {
        ...
        var Tam = this.Audio.Analizador.fftSize / 2;
    ...
    PintarPantalla_Solar : function(Buffer, Tick) {        
        ...
        Buffer.Context.fillStyle = "rgba(0, 0, 0, 0.5)";
        Buffer.Context.fillRect(0, 0, Buffer.Ancho, Buffer.Alto);
        var TamBuffer = this.Audio.Analizador.fftSize / 2;
    ...
    PintarPantalla_Circular : function(Buffer, Tick) { 
        ...
        Buffer.Context.fillRect(0, 0, Buffer.Ancho, Buffer.Alto);
        var TamBuffer = this.Audio.Analizador.fftSize / 2;
        var RadAvance = this.Constantes.TAU / this.Audio.Analizador.fftSize;
        var Rad = -Math.PI / 2, x = 0, y = 0;       
        var mx = (Buffer.Ancho / 2);
        var my = (Buffer.Alto / 2);
...
var EspectroAudible_Audio = function() {
    ...
    this.Iniciar = function(fftSize, Padre, FuncionCanPlay, FuncionEnded) {
        this.Analizador = this.Context.createAnalyser();
        this.Analizador.fftSize = fftSize;
    ...
    this.CargarCancion = function(PosCancion) {        
        ...
        if (this.CancionActual !== -1) { if (this.Cancion) { this.Cancion.pause(); } }
        else                           { this.AudioSourceDD.stop(0); }
        this.CancionActual = PosCancion;
        this.Cancion = new Audio();
        this.Cancion.controls = true;
        this.Cancion.crossOrigin = "anonymous";
        this.Cancion.src = "/Canciones/" + this.Canciones[PosCancion].Path;
    ...
    this.CanPlay = function() {
        if (this.CancionCargada !== true) {
            ...
            if (this.CancionActual !== -1) {
                this.AudioSource = this.Context.createMediaElementSource(this.Cancion);
                this.AudioSource.connect(this.Analizador);
                this.Analizador.connect(this.Context.destination);
                this.FuncionCanPlay();
            }
        }        
    };
    ...
    this.PlayPausa = function() {
        if (this.CancionActual !== -1) {
            ...
            if (this.Cancion.duration > 0 && !this.Cancion.paused) { this.Cancion.pause();  return false;  } 
            else                                                   { this.Cancion.play();   return true; }
        }
        else { // Drag & drop
            return false;
        }
    ...
    this.Adelante = function() {
        if (this.CancionActual !== -1) {  this.Cancion.pause(); }
        var p = 0;
    ...
    this.NuevoTiempoActual = function(NuevoTiempo) {
        if (this.CancionActual !== -1) { this.Cancion.currentTime = NuevoTiempo; }
        else                           { this.AudioSourceDD.context.currentTime = NuevoTiempo; }        
    };
    
    this.TiempoActual = function() {
        if (this.CancionActual !== -1) { return this.Cancion.currentTime; }
        else                           { return this.AudioSourceDD.context.currentTime; }
    };
    
    this.TiempoTotal = function() {
        if (this.CancionActual !== -1) { return this.Cancion.duration; }
        else {
            if (this.AudioSourceDD.buffer) { return this.AudioSourceDD.buffer.duration; }
            else                         { return 0; }
        }
    };
    ...
    this.CargarCancionDrop = function(Archivos) {    
        ...
        Reader.addEventListener('load', function(e) {
            this.Context.decodeAudioData(e.target.result, function(Buffer) {
                this.Padre.Cargando(false);
                this.CancionCargada = true;
                this.AudioSourceDD = this.Context.createBufferSource();
                this.AudioSourceDD.buffer = Buffer;
                this.AudioSourceDD.connect(this.Analizador);
                this.Analizador.connect(this.Context.destination);
                this.AudioSourceDD.start(0);                
                this.FuncionCanPlay();
            }.bind(this));
        }.bind(this));
